const get_header_html = (page, check_auth_user) => {
    ...
    if (check_auth_user === false) {
        button_profile = `<button title="Профиль" type="button" class="header__nav__button" id="header_profile_button" onclick="toggle_auth_block()">
    ...
    return `<div class="header__bg ${header_bg_class}" id="header__bg"></div>
                ...
                <div class="header__authBlock header__authBlock_hidden">
                    <h4 class="txt">Вход и регистрация только через Steam</h4>
                    <a href="" class="txt">Войти</a>
                </div>`;
}
